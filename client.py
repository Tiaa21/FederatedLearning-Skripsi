...
if __name__ == "__main__":
    ...
    optimizer = optim.AdamW(
        model.parameters(),
        lr=params.learning_rate,   # e.g. 1e-4 works well
        weight_decay=1e-4
    )
    ...
    scheduler = CosineAnnealingWarmRestarts(
        ...
        eta_min=1e-6    # minimal learning rate
    )
    ...
    while True:
        try:
            ...
            print(f"[CLIENT {args.site_id}] 📉 Current Learning Rate: {current_lr:.6e}")
        except Exception as e:
            print(f"[CLIENT {args.site_id}] ⚠️ Could not read LR: {e}")
        ...
        global_model = None
        adv_global_plain_path = f"agg/global_round{max(0, round_num-1)}.pth"       # plain
        adv_global_enc_path   = f"agg/global_round{max(0, round_num-1)}_enc.pth"   # ckks
        ...
        if params.mode == "Plain":
            ...
            # server menyimpan hasil agregasi ciphertext di file ini
        ...
        while True:
            try:
                ...
                if params.mode == "Plain":
                    ...
                    ctx_path = "agg/ckks_context.con"   # path konteks yang server share
