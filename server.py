...
def strip_epoch_prefix(key: str):
    clean_key = key
    # buang angka depan sebelum underscore
    if "_" in key and key.split("_", 1)[0].isdigit():
        clean_key = key.split("_", 1)[1]
    return clean_key
...
def aggregate_ckks(weights_list, HE_pub, noise=0.0):
    ...
    # Ambil semua key yang muncul di semua client
...
if __name__ == "__main__":
    ...
    for round_num in range(params.n_epochs):
        ...
        if not np.isnan(mean_val_loss):
            ...
            if mean_acc > best_acc or (mean_acc == best_acc and val_loss < best_val_loss):
        ...
        try:
            if round_num >= 2:
                old_round = round_num - 2

                # CKKS Cleanup
                if params.mode == "CKKS":
                    try:
                        for fname in os.listdir("agg"):
                            if f"_round{old_round}_" in fname and fname.endswith(".bin"):
                                os.remove(os.path.join("agg", fname))
                        print(f"[SERVER] Cleaned up CKKS ciphertext files from round {old_round}.")
                    except Exception as e:
                        print(f"[SERVER] CKKS cleanup error: {e}")

                for i in range(args.num_clients):
                    local_path = f"agg/site{i}_round{old_round}.pth"
                    metrics_path = f"agg/site{i}_round{old_round}_metrics.npy"
                ...
                print(f"[SERVER] Deleted client files from round {old_round} to save space.")
        except Exception as e:
            print(f"[SERVER] CKKS cleanup error: {e}")
